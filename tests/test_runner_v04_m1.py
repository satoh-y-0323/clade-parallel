...
"""
        ...
        assert re.fullmatch(r"[0-9a-f]{8}", uuid8_part), (
...
"""
    ...
    worktree_add_cmds = [
        cmd for cmd in git_commands_called
        if "worktree" in cmd and "add" in cmd
    ]
    assert len(worktree_add_cmds) >= 1, (
...
def test_run_manifest_skippedタスクのbranch_nameはNone(
...
"""
    ...
    assert hasattr(tr_b, "branch_name"), "Skipped TaskResult must have branch_name field"
    assert tr_b.branch_name is None, (
...
def test_RunResult_merge_resultsのデフォルトは空タプル():
    ...
    assert isinstance(rr.merge_results, tuple), (
