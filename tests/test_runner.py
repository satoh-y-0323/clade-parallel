...
def test_全タスク成功時にon_complete_webhookが送信される(fake_claude_runner, tmp_path):
    ...
    assert result.overall_ok is True
    # urlopen must be called exactly once (on_complete only).
    assert mock_urlopen.call_count == 1
    # Verify the URL passed to urlopen via the Request object.
...
def test_タスク失敗時にon_completeとon_failure両方が送信される(
    ...
    assert result.overall_ok is False
    # urlopen must be called twice: once for on_complete, once for on_failure.
...
def test_webhook送信失敗時は警告のみで終了コードに影響しない(
    ...
    """urlopen raises URLError → warning is printed to stderr; run still succeeds."""
    import urllib.error
...
def test_webhook未設定時はurlopen未呼び出し(fake_claude_runner, tmp_path):
    """No on_complete or on_failure configured → urlopen is never called."""
    from unittest.mock import patch
