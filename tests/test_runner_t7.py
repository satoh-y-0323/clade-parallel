...
def test_run_manifest_推移的skip_A失敗でBとCが両方skippedになる(
    ...
    assert tr_b.skipped is True, (
        f"task-b must be skipped but skipped={tr_b.skipped}"
    )
...
def test_run_manifest_独立タスクDはA失敗の影響を受けない(
    fake_claude_runner, tmp_path
):
    ...
    assert tr_b.skipped is True, (
        f"task-b should be skipped but skipped={tr_b.skipped}"
    )
...
def test_run_manifest_書込タスクありのときrun_manifest冒頭でrequire_git_rootが呼ばれる(
    ...
    def fake_execute_task(task: Any, claude_exe: str, *, git_root: Any = None) -> TaskResult:
        return TaskResult(
            task_id=task.id,
            agent=task.agent,
            returncode=0,
...
"""
    ...
    assert "runner error #1" in propagated, (
