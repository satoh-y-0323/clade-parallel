...
Verifies the fix for the bug where ``A(not resumed) -> B(resumed) -> C``
caused C to be submitted before A completed.
"""
...
def _make_read_only_task(
    ...
    *,
    ...
    """Build a minimal read-only Task for scheduler tests."""
    return Task(
        id=task_id,
        agent="code-reviewer",
        read_only=True,
...
# ---------------------------------------------------------------------------
# Test: A(not resumed) -> B(resumed) -> C
#   C must NOT run before A completes.
# ---------------------------------------------------------------------------


def test_resume_dep_chain_A_not_resumed_B_resumed_C_not_before_A() -> None:
    """A -> B(resumed) -> C: C must wait for A to complete, not run immediately.

    Bug scenario: when B is in resumed_task_ids, the scheduler used to
    pre-decrement C's indegree to 0 during the pre-populate phase, causing
    C to be submitted in the initial batch alongside A — before A completed.

    This test verifies that C is executed only AFTER A finishes, by recording
    the execution order.
    """
    ...
    # Record the order in which execute_fn is called.
    ...
    def execute_fn(task: Task) -> TaskResult:
        ...
        if task.id == "task-a":
            # Simulate A taking some time so we can observe if C jumps ahead.
            time.sleep(0.05)
            a_done_event.set()
        elif task.id == "task-c":
...
def test_resume_dep_chain_A_resumed_B_not_C_not() -> None:
    ...
    # task-a must not be executed (resumed).
